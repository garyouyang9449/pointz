...
export async function registerAuthRoutes(app) {
    app.post("/auth/signup", async (request, reply) => {
        ...
        const token = await reply.jwtSign({ sub: created.id, email: created.email }, { expiresIn: "7d" });
    ...
    app.post("/auth/login", async (request, reply) => {
        ...
        const token = await reply.jwtSign({ sub: row.id, email: row.email }, { expiresIn: "7d" });
        return {
            token,
            user: {
                id: row.id,
