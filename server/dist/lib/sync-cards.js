 ...
 */
const categoryEnum = z.enum(categoryIds);
...
async function loadDataset() {
    ...
    const file = process.env.CARDS_DATA_FILE ??
...
async function syncCards(dataset, { prune }) {
    const stats = {
        cardsUpserted: 0,
        rulesUpserted: 0,
        rulesDeleted: 0,
    ...
    try {
        ...
        for (const card of dataset.cards) {
            await client.query(`INSERT INTO cards (id, issuer, name, network, annual_fee)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (id) DO UPDATE SET
           ...
           annual_fee = EXCLUDED.annual_fee`, [
            ...
            const deleted = await client.query(`DELETE FROM reward_rules
         WHERE card_id = $1 AND category <> ALL($2::text[])`, [card.id, keepCategories]);
            stats.rulesDeleted += deleted.rowCount ?? 0;
            for (const rule of card.rewardRules) {
                await client.query(`INSERT INTO reward_rules
        ...
        if (prune) {
            ...
            const deleted = await client.query(`DELETE FROM cards WHERE id <> ALL($1::text[])`, [keepIds]);
...
async function main() {
    const prune = process.argv.includes("--prune") || process.env.CARDS_SYNC_PRUNE === "true";
    const dataset = await loadDataset();
    // eslint-disable-next-line no-console
    console.log(`Dataset version=${dataset.version} cards=${dataset.cards.length}` +
        (prune ? " (prune mode: unknown cards will be deleted)" : ""));
    const stats = await syncCards(dataset, { prune });
    // eslint-disable-next-line no-console
    console.log(`Sync complete. cardsUpserted=${stats.cardsUpserted} ` +
