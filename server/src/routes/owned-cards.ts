...
async function getUserCardIds(userId: string): Promise<string[]> {
  const rows = await db
...
async function assertCardsExist(ids: string[]): Promise<void> {
...
export async function registerOwnedCardsRoutes(app: FastifyInstance): Promise<void> {
  app.get(
    "/me/owned-cards",
  ...
  app.delete(
    ...
    async (request, reply) => {
      ...
      await db
        ...
        .where(
          and(eq(userOwnedCards.userId, userId), eq(userOwnedCards.cardId, cardId))
        );
